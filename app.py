...
st.sidebar.markdown("### ⚙️ Executive Command (Master HP)")
...
def save_github_brain(brain_data, current_sha=None):
    ...
    if token and current_sha:
        ...
        payload = {"message": "Vault: Full Master Restoration", "content": encoded, "sha": current_sha}
        try:
...
.avatar-circle {{
    ...
    height: 75px;
    border-radius: 50%;
    background: linear-gradient(135deg, #251833 0%, {current_theme['primary']} 100%);
    display: flex;
    align-items: center;
    ...
    box-shadow: 0 0 20px {current_theme['glow']};
}}
...
<div style="text-align: center; margin-bottom: 40px; padding-top: 15px;">
    ...
    <p style="color: #9494a6; font-weight: 600; letter-spacing: 6px; margin-top: 12px; text-transform: uppercase; font-size: 0.88rem;">Managed by Mike Donna // Full Master Apex Terminal</p>
</div>
""", unsafe_allow_html=True)
...
with tab6:
    st.markdown('<div class="exec-card">', unsafe_allow_html=True)
    st.markdown('<h3>Core Roster // Professional Player Avatars & Live News Feed</h3>', unsafe_allow_html=True)
    st.markdown("<p style='color:#a1a1aa; font-size:0.85rem; margin-bottom:20px;'><b>Data Sourcing:</b> Professional player registry & Rotowire NFL Injury Wire.</p>", unsafe_allow_html=True)
    
    col_p1, col_p2, col_p3, col_p4 = st.columns(4)
    with col_p1:
        st.markdown("""
        <div class="pro-avatar-card">
            <div class="avatar-circle">KW</div>
    ...
    with col_p2:
        ...
        <div class="pro-avatar-card">
            ...
            <span class="source-badge" style="margin-top:8px; display:inline-block;">LOCKED // TARGET SHARE</span>
        </div>
        """, unsafe_allow_html=True)
    with col_p3:
        st.markdown("""
    ...
    with col_p4:
        st.markdown("""
        <div class="pro-avatar-card">
            <div class="avatar-circle">DH</div>
    ...
    parlays_detailed = [
        ...
        {"title": "Tier 5 // 6-Leg Cross-Conference Over/Under", "odds": "+7,500", "payout": "76.00x Return", "legs": ["Leg 1: New England @ Seattle Under 44.5 Total", "Leg 2: San Francisco vs LA Rams Over 48.5 Total", "Leg 3: Chicago @ Carolina Under 44.5 Total", "Leg 4: Baltimore @ Indianapolis Over 47.5 Total", "Leg 5: Tampa Bay @ Cincinnati Over 50.5 Total", "Leg 6: Denver @ Kansas City Over 42.5 Total"]},
