...
def render_parlay_card(ticket_id, tier, odds, color, legs_json, weight_class):
    try:
    ...
    <div style="background: linear-gradient(145deg, #171d2b 0%, #0b0e14 100%); border: 1px solid {color}40; border-top: 4px solid {color}; border-radius: 12px; padding: 18px; margin-bottom: 24px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #212838; padding-bottom: 12px; margin-bottom: 12px;">
            ...
            <div style="text-align: right;">
                ...
                <span style="color: #8b949e; font-size: 0.8rem; font-weight: 700;">{weight_class}</span>
            </div>
        </div>
        {legs_html}
        <div style="margin-top: 18px; display: flex; justify-content: space-between; gap: 12px;">
            <button onclick="navigator.clipboard.writeText('{ticket_id}: {odds} on {weight_class}')" style="flex: 1; background: #0b0e14; color: #8b949e; border: 1px solid #212838; padding: 10px; border-radius: 6px; font-size: 0.8rem; font-weight: 800; cursor: pointer;">
                📋 COPY SLIP
            </button>
...
def render_prizepicks_underdog(conn=None):
    ...
    try:
        ...
        df = pd.read_sql("SELECT ticket_id, weight_class, odds, border_color, ticket_json, created_at, confidence_score FROM theoretical_bets
        if not df.empty:
            for _, row in df.iterrows():
                st.markdown(render_parlay_card(row['ticket_id'], row['weight_class'], row['odds'], row['color'], row['ticket_json'], row['weight_class']), unsafe_allow_html=True)
        else:
