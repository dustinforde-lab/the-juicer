...
st.set_page_config(page_title="The Juicer // Agent Analytics", layout="wide", page_icon="🧠")
...
try:
    ...
    with col1:
        st.markdown("<h3 style='color:#00e5ff;'>🧠 MIKE'S RECIPE WEIGHTS (MACHINE LEARNING)</h3><br>", unsafe_allow_html=True)
        ...
        st.markdown("<br><h3 style='color:#ffaa00;'>📉 DONNA'S VIBE CHECK (CHALK & LEVERAGE)</h3><br>", unsafe_allow_html=True)
        try:
    ...
    st.error(f"Failed to connect to War Room database: {e}. Has the orchestrator run yet?")
