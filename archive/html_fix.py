...
print("\n" + "="*65)
print(" 🛠️ FIXING HTML RENDERING & SLIP PARSING IN UI_ADDRESSES.PY")
print("="*65)
...
def _render_slip_feed(platforms, title, color):
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if legs:
                for l in legs:
                    p_name = l.get('player_name', l.get('player', 'Unknown Player'))
...
try:
    with open("ui_addresses.py", "w", encoding="utf-8") as f:
        f.write(fixed_ui_addresses.strip())
    print("  ✅ ui_addresses.py updated with proper unsafe_allow_html=True parsing and robust JSON fallback.")
except Exception as e:
    print(f"  ❌ Failed to update ui_addresses.py: {e}")

print("\n" + "="*65)
print(" 🚀 REBOOTING STREAMLIT WITH CLEAN UI RENDERING...")
print("="*65 + "\n")